...
window.addEventListener('click', (event) => {
  ...
  console.log(event.clientY, player.position.y + player.height / 2)
  console.log(event.clientX, player.position.x - 120, player.position.x + player.width + 120)
...
function animate() {
  ...
  projectiles.forEach((projectile, index) => {
    ...
    if (projectile.position.x - projectile.radius < 0 || projectile.position.x + projectile.radius > canvas.width
      || projectile.position.y - projectile.radius < 0 || projectile.position.y + projectile.radius > canvas.height) {
